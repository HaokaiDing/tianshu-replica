...
const help=()=>console.log("tianshu init <input> [--title T] [--episodes 30|60] [--source-outline outline.md] [--contract production-contract.json] | plan <run> | status <run> | metrics <run> | approve <run> | run <run> | resume <run> | produce <run> | review <run> | repair <run> | storyboard <run> | storyboard-review <run> | approve-delivery <run> | return <run> <note> | deliver <run> | lock-status <run> | unlock-stale <run>");
...
try { if(cmd==="init"){const input=readText(args[0]);const title=flag("--title")||input.split("\n")[0].slice(0,50);const contractFile=flag("--contract"),productionContract=contractFile?JSON.parse(readText(contractFile)):undefined;const sourceFile=flag("--source-outline");if(args.includes("--source-outline")&&(!sourceFile||sourceFile.startsWith("--")))throw new Error("--source-outline requires a Markdown file");const {id,manifest}=createRun(ROOT,{title,episodes:flag("--episodes")||30,input,...(sourceFile?{sourceOutline:readText(sourceFile)}:{}),...(productionContract?{productionContract}:{})});console.log(JSON.stringify({id,state:"draft",productionRoute:manifest.productionRoute}));}
