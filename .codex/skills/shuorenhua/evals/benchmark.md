...
| status | SF-02, SF-09, SF-15, SF-21, SF-25, SF-30, SF-46, SF-48, SNF-03, SNF-06, SNF-10, SNF-13, SNF-19, SNF-21, SNF-28, SNF-33, SNF-36 | - | - | SF-23, SNF-18 |
| docs | SF-04, SF-07, SF-26, SF-57, SF-58, SF-59, SF-60, SF-61, SNF-01, SNF-02, SNF-04, SNF-05, SNF-08, SNF-09, SNF-14, SNF-20, SNF-34, SNF-39, SNF-40, SNF-43, SNF-45, SNF-46, SNF-47, SNF-48, SNF-49 | SF-18, SF-47, SNF-15, SNF-23, SNF-37 | SNF-50 | SF-22, SF-24, SF-27, SNF-17 |
| public-writing | SF-03, SF-05, SF-08, SF-10, SF-12, SF-13, SF-16, SF-20, SF-28, SF-32, SF-33, SF-35, SF-42, SF-43, SF-44, SF-49, SF-50, SF-51, SF-52, SF-54, SF-55, SF-56, SNF-12, SNF-24, SNF-25, SNF-27, SNF-41, SNF-44 | SF-34, SF-39, SF-40, SF-41, SF-53, SNF-26, SNF-29, SNF-30, SNF-31, SNF-32, SNF-38, SNF-42 | SF-14 | - |
| code-context | - | - | - | SNF-22 |
...
### SF-58 | docs / api-reference | API 说明先讲价值，调用条件埋在后面
...
**预期**：按 `api-reference` scene pack 处理。删掉 `面向未来 / 新一代 / 智能化能力矩阵 / 赋能 / 双重跃迁`，保留原文明说的「用于提交异步任务」与调用约束；可改成先列 `POST /v1/jobs`、`Idempotency-Key` 和 `timeout_ms` 的条件。`POST /v1/jobs`、`Idempotency-Key`、`1–64`、`ASCII`、`timeout_ms`、`30000`、`1000`、`120000` 必须逐字保留，`timeout_ms` 仍是可选字段。把参数表述改成宣传段落、改字段名/大小写/数字、把可选改成必填，或补原文没有的鉴权方式，记 `❌`。

### SF-59 | docs / api-reference | 缺失的默认值和恢复时间不能靠确定口吻补齐
...
**预期**：按 `api-reference` scene pack 处理。保留上游规范明确给出的 endpoint、参数、枚举、状态码和错误码；删除把缺失合同补成事实的建议，或把 `30 秒内恢复 / 默认退避间隔 / 重试三次 / locale 默认 zh-CN` 明确列为待确认，不能继续当作已证实，也不能擅自换成其他数值或恢复策略。`GET /v2/exports/{export_id}`、`locale`、`zh-CN`、`en-US`、`503`、`EXPORT_QUEUE_BUSY` 必须逐字保留。只做顺句、不暴露四个待确认项，记 `⚠️`；删掉错误码、枚举或补出退避算法，记 `❌`。

### SF-60 | docs / faq | FAQ 应先回答，不用客服寒暄重复问题
> ## 忘记密码后应该怎么办？
>
...
**预期**：按 `faq` scene pack 处理。删掉假共情、`很多用户都会遇到`、重复问题和 `便捷而完善`，答案直接从操作步骤开始；保留标题的问题范围、`注册邮箱`、`15 分钟内有效`、企业 `SSO` 和 `联系企业管理员`。不能把 15 分钟改成别的期限，不能把企业账号也导向邮件重置，不能补原文没有的客服电话。

### SF-61 | docs / faq | 无源保证不能掩盖限制和支持边界
> ## 升级后一定不会丢数据吗？
>
> 大家都知道，我们的升级流程一直非常可靠，通常可以确保所有用户的数据百分之百安全。v3 客户端不支持从 v1 直接升级；请先升级到 v2.8，再导出备份，最后安装 v3。遇到问题时，官方一般都会在 2 小时内恢复。

**预期**：按 `faq` scene pack 处理。先直接回答“原文无法证明一定不会丢数据”，删掉 `大家都知道 / 一直非常可靠 / 通常可以确保 / 百分之百安全` 的无源保证；完整保留 `v1 → v2.8 → 导出备份 → v3` 的顺序和“不支持直接升级”的硬限制。`2 小时内恢复` 没有来源，应标为待确认，不能改成正式 SLA，也不能换一个时长。删步骤、软化“不支持”、扩大成零风险承诺，记 `❌`。
...
> 钉住 [结构反模式](../references/structures.md) 第 21–25 条。这五类的共同点是：单个词都不违规，问题在句子和段落的形状上，词表拦不住。

### SF-48 | status | 动词名词化堆砌
> 本周对部署流程进行了梳理，实现了发布耗时的下降。同时完成了对灰度策略的调整，起到了降低回滚频率的作用。下周计划开展监控告警的优化工作。

**预期**：钉住第 21 条。四处名词化全部还原成直接动词：`对部署流程进行了梳理` → `梳理了部署流程`，`实现了发布耗时的下降` → `发布变快了`，`完成了对灰度策略的调整` → `调了灰度策略`，`起到了降低回滚频率的作用` → `回滚少了`，`开展监控告警的优化工作` → `优化监控告警`。原文没给任何数字，**不得编造耗时、频率或百分比**；`status` 场景可标注缺具体口径。保留三件事的时间归属（本周做了三件、下周计划一件），不能把下周的计划并进本周已完成。这是 `status`，不要改成聊天腔。只把「进行了」换成「做了」而名词结构还在（`对部署流程做了梳理`），记 `⚠️`。

### SF-49 | public-writing | 同义词躲避
> 我去年开始学做木工。这门手艺上手比想象中难，光是磨凿子就练了两个月。后来这项技能倒是派上了用场，家里的柜子门松了，自己就能修。

**预期**：钉住第 22 条。`木工 → 这门手艺 → 这项技能` 是同一对象逐次升格换词，统一回 `木工`，或第二次起改用代词（`它`）。判断依据是三个说法没有任何一个带来新信息或新限定。`去年`、`两个月`、`磨凿子`、`柜子门松了` 是受保护片段。反向失败：为了「避免重复」再造一个新说法（`这项本事`、`这门活儿`），记 `❌`。把三句并成一句来消除重复，记 `⚠️`——问题是换词不是重复，并句没解决它。

### SF-50 | public-writing | 连词过密
> 因为公司搬到了新园区，所以我的通勤一下子多了四十分钟。同时地铁还多换乘一次，因此我认真考虑过搬家。但是新园区附近的房租比原来贵一千二，而且户型普遍偏小。所以我最后没搬，并且开始试着错峰上班，早走一小时，路上能省二十分钟。

**预期**：钉住第 23 条。`因为 / 所以 / 同时 / 因此 / 但是 / 而且 / 所以 / 并且` 八个连词在约 110 字里，删掉一半，事理靠语序自己接（`公司搬到新园区，我的通勤一下子多了四十分钟`）。这是 `public-writing` 叙事，本条适用。`四十分钟`、`一千二`、`一小时`、`二十分钟`、`换乘一次` 必须逐字保真，删连词时不得连带删掉任何一个数字或它修饰的对象。保留转折关系本身（房租贵和户型小是没搬的原因），只删显性连接词。删到读不通、需要读者自己补逻辑，记 `⚠️`。

### SF-51 | public-writing | 装饰性细节
> 凌晨两点，办公室的灯只剩他工位那一盏。窗外飘着小雨，桌上的咖啡早就凉透了。他盯着屏幕看了很久，手指在触摸板上停了几秒，最终还是把方案删掉了。这个项目他做了三个月。

**预期**：钉住第 24 条。`凌晨两点`、`只剩一盏灯`、`窗外飘着小雨`、`咖啡凉透了`、`盯着屏幕看了很久`、`手指停了几秒` 六处同时满足两个条件——没有来源，且删掉后文的事实与因果不变——应当删除。**必须保留**：`把方案删掉了` 和 `这个项目他做了三个月`，这两条是仅有的实际信息。删完段落变短是正常的，不要用新的场景描写补足长度。反向失败：把装饰细节换成另一套装饰细节（`他揉了揉眼睛` 之类），记 `❌`；把 `三个月` 删掉或改成 `很久`，记 `❌`。

### SF-52 | public-writing | 借喻场混用
> 这条赛道的护城河正在快速坍塌。过去三年积累的技术壁垒，被开源方案一夜之间冲垮，整个行业都被卷进了这波浪潮。团队得重新点燃斗志，在下一个十字路口找到自己的位置。

**预期**：钉住第 25 条。约 100 字里混了五套借喻场：道路竞赛（`赛道`、`十字路口`）、战争攻防（`护城河`、`壁垒`）、建筑灾害（`坍塌`）、海洋航行（`冲垮`、`浪潮`）、温度（`点燃`）。远超「800 字内 3 套」的判据，应全部还原成本义：`门槛在降低`、`开源方案追上了自研`、`团队要重新找差异点`。`三年`、`开源方案` 是受保护片段。还原后如果只剩空话，说明原句本来就只有比喻没有信息，允许变短并标注缺口径，**不得补出原文没有的产品、指标或竞争对手**。用一套新比喻统一替换全部旧比喻，记 `⚠️`——问题是拿比喻替代说明，不是比喻不统一。

### M. 标点腔长文与 bounded 交互（v2.1.1 欠账，v2.3.0 合并交付）

> 标点腔此前只有 short 覆盖（SF-43 / SNF-34），长文里破折号和 `bounded` 删除清单怎么交互没有钉住。本节补这个空白。

### SF-53 | public-writing / long / bounded | 标点腔跨段高密度 + 整句空话，两类动作不能混
...
1. **破折号是句内动作，句内改，不进删除清单。** 全文 7 处 `——` 跨 4 段且首句起手，命中结构 20 的密度信号。按语义改回冒号、逗号或断句，每段至多留一处真正承担插入的。把带破折号的整句丢进「建议删除（待确认）」清单，记 `❌`。
2. **整句空话照常进删除清单。** 两句剥掉引导词后不剩实质：`研究表明，超过七成的开源维护者在两年内出现倦怠`（无源引用）、`说到底，维护开源是一场与自己耐心的长跑`（价值拔高收尾）。进清单交用户拍板，不直接删、不软化成另一种说法。
3. **实句一句都不动。** `三百多个 star`、`一百四十条`、`一百一十条`、`七次`、`四轮`、`六成`、`两成`、`二十分钟`、`三个字段`、`三类需求`、`SCOPE.md`、`Windows 路径`、`GUI` 全部逐字保真。首句 `很多人以为……其实不是` 是单处翻案骨架，未达密度阈值，去掉破折号即可，不必改写结构。

`bounded` 不适用留存率硬下限（删整句空话会降字数），但除清单内两句外的信息点必须全部可追溯。把四段并成两段或重排段落，记 `❌`。

### N. v2.3.0 合并批次：词表、抒情词层与量化保真

### SF-54 | public-writing | 新补词在公开写作里的典型命中
> 接下来要洞察用户卡点，通过产品和运营联动，抢占下一条增长赛道。

**预期**：`洞察 / 卡点 / 联动 / 赛道` 在这里都没有承担正式术语或字面实义，应改回具体动作，例如「弄清用户卡在哪里，让产品和运营一起处理，再找新的增长方向」。必须保留 `产品` 与 `运营` 是两个参与方，不能合并主体，也不能补原文没有的用户调研方法、业务指标或市场结论。只换成另一组商业黑话，记 `⚠️`。

### SF-55 | public-writing | 抒情词给抽象概念穿衣服，同段 2+ 应处理
> 我们都在寻找一个让记忆「安放」的地方。那些被时间「剥开」的褶皱里，还藏着抵达明天的「微光」。

**预期**：`安放 / 剥开 / 褶皱 / 抵达 / 微光` 同段聚集，描写对象是 `记忆 / 时间 / 明天` 等抽象概念，命中 Tier 2 抒情词层；三处 `「」` 同时是自造高亮短语的 residual 观测信号。应压回直接陈述，例如「我们会回想过去，也会想以后怎么办」，不要换一组抒情词继续包装。原文没有具体事件或结论，允许变短；不得补人物经历、时间或因果。

### SF-56 | public-writing | 非关键信息里的量化歧义可以压缩，不得改成新关系
> 这次只改了首页排版：标题字号缩小了 3 倍，卡片间距也调了一下，现在看着没那么挤。

**预期**：`缩小了 3 倍` 有歧义，改写侧不能替作者选成 `缩小到原来的 1/3`、`减少了 2/3` 或其他新数量关系。本句的倍数不承担关键测量结论，`public-writing` 下可压成「标题字号调小了」，保留 `首页排版 / 标题字号 / 卡片间距 / 没那么挤` 的关系。原样保留并明确提示歧义可记 `⚠️`；自行推导出新比例，记 `❌`。

### SF-57 | docs | 量化歧义走 audit-only，标注待确认
> 压测结果显示，错误率降低了 2 倍，详情见表 3。

**预期**：`降低了 2 倍` 有歧义；`docs` 默认走 `audit-only`，保留原句并明确标注「降低 2 倍的数量关系待确认」。不得改成 `降到原来的 1/2`、`降低 50%` 或自行补基线、样本量、测量期限。`压测结果 / 错误率 / 表 3` 都要保留；把歧义数字删掉后改成「错误率明显降低」会丢关键结果，同样不通过。
...
### SNF-39 | docs | 承担逻辑的连词密度，不该按 public-writing 判据删
...
**理由**：钉住 [结构反模式](../references/structures.md) 第 23 条的场景限定，**与 SF-50 构成同密度对照对**。实测连词密度 81.08/千字（111 字里 9 处：`如果`×3、`否则`×2、`因为`×2、`所以`、`此外`），**比 SF-50 的 80.00/千字还高**，结论却相反——SF-50 是 `public-writing` 叙事，该删一半；本条是 `docs`，一个都不能删。每个连词都在承担条件、因果或并列，删掉任何一个都会让操作步骤读错或产生歧义。这一对用例是「按场景判、不按总量判」的直接证据，比标定分布更硬。`GTID`、`sql_log_bin`、`binlog`、`SET FOREIGN_KEY_CHECKS=0`、`外键约束` 都是受保护片段。为了「降低连词密度」把条件句改成并列短句，或删掉 `否则` 这类后果说明，记 `❌`。

### SNF-40 | docs | 稳定术语里的名词结构，不是名词化病灶
> 增量编译只在源文件的 mtime 变化时触发。执行计划生成阶段会读取上一次的依赖图，对未变更的模块直接复用产物。首次构建不适用，需要完整走一遍全量编译。

**理由**：钉住第 21 条的保留条件。`增量编译`、`执行计划生成`、`全量编译` 是构建系统里的稳定术语，本身就是名词，不是「空动词 + 动名词」的名词化结构（对比 SF-48 的 `对部署流程进行了梳理`）。判据是能不能还原出一个被隐藏的动作：`增量编译` 还原不出，它就是这个东西的名字。把 `执行计划生成阶段` 改成 `生成执行计划的阶段` 属于无意义改写，记 `⚠️`；改成 `编译的时候` 这类丢术语的说法，记 `❌`。`mtime`、`依赖图`、`产物` 同为受保护片段。

### SNF-41 | public-writing | 借喻词的字面用法与行业术语
> 我们把构建产物推到私有代码仓库，CI 从那里拉取。搜索引擎的索引更新有延迟，所以刚发布的文档要等十几分钟才能搜到。这家公司真正的护城河是十年积累的专利池，不是产品本身。

**理由**：钉住第 25 条的保留条件与脚本侧的字面用法排除表。`代码仓库`（仓储场）、`搜索引擎`（机器器官场）、`护城河`（战争攻防场）分属三套借喻场，按数量判据「800 字内 3 套」名义命中，但三个都不是借喻：前两个是字面技术术语，`护城河` 是投资语境的行业稳定术语，且这里带着 `十年积累的专利池` 这个具体依据。`hard_metrics.py` 的 `METAPHOR_LITERAL_PREFIX` 已把 `代码仓库` / `搜索引擎` 排除，本用例同时验证规则侧和脚本侧的一致性。`CI`、`索引`、`十几分钟`、`十年`、`专利池` 是受保护片段。把 `护城河` 改成 `优势` 记 `⚠️`（丢了「难以被跨越」这层意思），把 `代码仓库` 改成 `代码库` 或 `存储位置` 记 `❌`。末句的 `不是产品本身` 是单处二元对比，未达密度阈值，一并放行。
...
### SNF-47 | docs / api-reference | 完整参数约束和速率限制应整体放行
...
**理由**：这是已经可扫描的 API reference，字段、required/optional、默认值、范围、速率限制、状态码和恢复信号完整，不含宣传或元评论。应整体 `no-op`。`POST /v1/messages`、`application/json`、`recipient_id`、`ttl_seconds`、`3600`、`60–86400`、`API key`、`120`、`429 RATE_LIMITED`、`Retry-After` 必须逐字保留；改大小写、换字段叫法、把表述叙事化、删除 429 或自行换算单位，均记 `❌`。

### SNF-48 | docs / api-reference | 系统主语、被动语态和接口特定状态语义不是 AI 味
> 当 `If-Match` 与资源当前的 `ETag` 不一致时，请求会被服务器拒绝并返回 `412 PRECONDITION_FAILED`。客户端必须重新获取资源，再使用新的 `ETag` 提交更新；不要自动覆盖服务端版本。

**理由**：被动语态在这里准确表达服务器行为，条件顺序、责任主体和恢复动作都清楚。`If-Match`、`ETag`、`412 PRECONDITION_FAILED` 是 protected spans，`必须重新获取` 与 `不要自动覆盖` 是硬约束。不得为了“主动、口语、简洁”改主体、删条件、软化约束或把 412 机械改写成别的状态说明；整体应 `no-op`。

### SNF-49 | docs / faq | 已经直接的排障 FAQ 不需要补安抚或改术语
...
**理由**：问题是用户会实际检索的问法，答案直接给步骤、分支和限制。`config file not found`、`shuorenhua config path`、`config.example.toml`、`SHUORENHUA_CONFIG`、`Windows` 都必须逐字保留，最后一句的“不读取”不能软化。整体应 `no-op`；不得补客服寒暄、改命令、删分支或虚构默认路径。

### SNF-50 | docs / interview / mixed | 有问号和 Q/A 结构不等于 FAQ
> **Q：你为什么保留文章里的重复？**
>
> **A：** 我写长文时常常会回到同一句话。那不是为了照顾搜索，也不是因为忘了前面写过什么；有时我只是还没想完。编辑可以提醒我哪里重复，但最后删不删，我想自己决定。

**理由**：这是采访式的作者自述，不是产品 FAQ 或排障条目。重复、口语问句和 `Q/A` 结构共同承担说话人的节奏与态度，应整体 `no-op`。不得套用 FAQ 的“结论先行”把回答压成操作说明，也不得把标题陈述化、删掉第一人称限定，或把两处 `不是` 合并成通用规则。

### SNF-38 | public-writing / long | 亲历叙事里的琐碎细节，不是装饰性细节
...
**理由**：**本条是 SF-51 的对照组，钉住 [结构反模式](../references/structures.md) 第 24 条的两条判据必须同时成立。** 表面看，`周六`、`十一点半`、`卷帘门拉下来一半`、`站了十分钟`、`不到一分钟`、`十二点二十` 和 SF-51 里被删掉的 `凌晨两点`、`咖啡凉透了` 一样琐碎，但两条判据一条都不成立：

1. **有来源**：全篇第一人称亲历，属于「用户自己提供的经历细节」，第 24 条保留条件第一项明写「哪怕琐碎也保留」。
2. **删掉后文会变**：这些细节串成一条完整因果链——周六只开半天所以差点没修成，靠隔壁老板打电话才开门，师傅超时二十分钟收尾又不肯加钱，最后一句「下次别赶周六来」回扣开头。删掉任何一环，「为什么这次修成了」就断了。

`四百二`、`三十`、`十分钟`、`二十分钟`、`十二点二十`、`十一点半`、`排线松了`、`不收线上支付` 全是受保护片段。把任一时间、金额或动作当装饰细节删掉，记 `❌`；把三段压成「师傅帮我修好了，只花了三十」这类摘要，记 `❌`——那正是第 24 条最危险的误杀方向。整体应 `no-op` 或仅做极轻微润色。

### SNF-42 | public-writing / long | 文学摘录里承担刻意文体的破折号
> ——他后来再没提过那年冬天的事。
>
...
**理由**：**本条是 SF-53 的对照组。** 两条都是长文、都是跨段破折号高密度、都以破折号起手，按结构 20 的密度判据同样超标（本条 6 处跨 4 段），但结论相反。区别在两点，都写在结构 20 的保留条件里：

1. **引用原文。** 文末 `——摘自《城郊纪事》，一九九八年冬` 标明了体裁与出处，整段是引用，不是待清理的模型输出。
2. **破折号承担刻意文体。** 这里的每一处都在制造停顿和语气延宕，是叙述节奏的一部分，尤其结尾 `不生炉子了——没有炉子可生` 的转折全靠这个停顿托住。改成逗号或句号，文体就塌了。

`五点`、`十块`、`一九九八年`、`《城郊纪事》` 是受保护片段。把破折号按密度判据机械替换，记 `❌`；删掉出处标注，记 `❌`；把文学叙述改写成信息密度更高的说明文，同样记 `❌`。整体应 `no-op`。判据顺序是「先查保留条件，再算密度」，不是反过来。

### SNF-43 | docs / project-style-guide | 项目术语表要求保留通常命中词
> 项目 style guide 规定：网关收到 `trace_id` 后原样透传给下游服务。`透传` 是本项目正式术语，请勿替换。

**理由**：用户当前要求与项目 style guide 优先于通用词表。`透传` 虽然通常命中商业黑话表，但这里是项目正式术语，而且规则明确要求保留；`trace_id`、`网关`、`下游服务` 同为受保护内容。应整体 `no-op`。把 `透传` 改成 `传递`，或只保留说明句却改动示例句，均记 `❌`。

### SNF-44 | public-writing / fiction | 抒情词描写具体事物，文学场景放行
> 她把药盒安放在床头，剥开一只橘子。母亲从厨房问：「刀口锋利吗？」她答：「锋利，别碰。」父亲又问：「围巾在哪？」窗外的微光落在柔软的围巾上。

**理由**：`安放 / 剥开 / 微光 / 柔软 / 锋利` 同段超过 Tier 2 密度，但描写对象分别是药盒、橘子、光线、围巾和刀口，全部承担实体实义；整段还是小说场景，命中两层保留条件。三处 `「」` 是人物对白，不是自造高亮短语，也应放行。整体应 `no-op`。为了降密度改成抽象同义词、删掉动作、改动对白或把文学叙述压成说明文，均为误杀。

### SNF-45 | docs | 百分点与相对增幅写法正确
> 转化率从 20% 升到 30%，提高了 10 个百分点，相对增幅为 50%。

**理由**：起止值、百分点变化和相对增幅都已写清，数量关系正确且无歧义。`20% / 30% / 10 个百分点 / 50%` 必须逐字保留，整体 `no-op`。删掉其中任何一个值、把百分点改成百分比，或因新规则开始重新计算数字，均记 `❌`。

### SNF-46 | docs | 带单位、基数和测量结果的正常数据句
> 本次共测 12000 次请求，接口 p95 延迟从 480ms 降到 160ms，减少 320ms。

**理由**：样本基数、指标、起止值、单位和差值完整，数量关系没有歧义。`12000 次请求 / p95 / 480ms / 160ms / 320ms` 都是 protected spans，必须逐字保留，整体 `no-op`。不得为了「统一量化口径」改数字、换单位、删基数或补原文没有的百分比。
...
- `篇章级病灶` 样本（SF-48 到 SF-52 / SNF-38 到 SNF-41）额外要求：这五类问题都在句子和段落的形状上，改法是还原而不是替换。**名词化**还原成直接动词，原文没有的数字不得补（SF-48）；**同义词躲避**统一回最具体的那个说法，为了「避免重复」再造一个新说法记 `❌`（SF-49）；**连词过密**只在 `public-writing` 叙事上判，`docs` 一个都不能删——SF-50（80.00/千字，该删）与 SNF-39（81.08/千字，不该删）是同密度对照对，按总量判必错；**装饰性细节**必须「无来源」与「删后文不变」两条同时成立才删，SF-51 与 SNF-38 是对照对，第一人称亲历里的琐碎细节一律保留，把亲历叙事压成摘要记 `❌`；**借喻场混用**还原本义、不换一套新比喻，字面用法（`代码仓库` / `搜索引擎`）与行业术语（投资语境的 `护城河`）按 SNF-41 放行
- `Scene Packs` 样本额外要求：先保留大场景边界和 protected spans，再按 `README / release-note / forum-post / issue-reply / api-reference / faq` 的发布目的收束语气；API reference 的 method、path、字段、类型、required/optional、默认值、范围、单位、枚举、状态码、错误码和恢复动作不得漂移，也不得补原文没有的接口合同；FAQ 不得扩大问题范围、软化否定/限制、改期限或支持承诺，也不能把访谈问答误判成 FAQ。任一受保护内容改变均记 `❌`
...
- `标点腔长文` 样本（SF-53 / SNF-42）额外要求：判据顺序是**先查保留条件、再算密度**。破折号是句内动作，在 `bounded` 下句内改、不进删除清单，同段整句空话照常进清单，两类动作不能混（SF-53）；引用体裁与承担刻意文体的破折号即使密度超标也放行，机械替换记 `❌`（SNF-42）
- `v2.3.0 合并批次新判据` 样本（SF-54 到 SF-57 / SNF-43 到 SNF-46）额外要求：项目 style guide 对正式术语的保留要求优先于通用词表（SNF-43）；抒情词必须按搭配对象与场景判，抽象概念上的聚集要处理、实体与文学场景放行（SF-55 / SNF-44）；量化歧义不得被擅自修成一种新关系，正常的百分点、百分比、单位、基数和测量结果一个字都不能动（SF-56 / SF-57 / SNF-45 / SNF-46）
