...
export function seriesReviewSubmissionTool({ stage, totalEpisodes, requireP2Disposition = true, onSubmit }) {
  return defineTool({
    ...
    async execute(_id, params) {
      try {
        ...
        return { content: [{ type: "text", text: `REJECTED ${error.message}. For each P2, set disposition to repair or accepted_non_blocking and resubmit.` }], details: { accepted: false }, terminate: false };
      }
      onSubmit(params);
...
export async function checkpointWindowReview(file, { systemPrompt, prompt }, runReview) {
  if (fs.existsSync(file)) {
    const checkpoint = readJson(file);
    if (checkpoint.systemPrompt === systemPrompt && checkpoint.prompt === prompt) return checkpoint.result;
  }
...
export function reviewPrompt(stage, contractText, manifest = {}) {
...
async function reviewWindow(runDir, stage, from, to, contractText, cycle) {
  ...
  return checkpointWindowReview(checkpointFile, { systemPrompt, prompt }, async () => {
    let submitted = false;
    let findings = [];
...
async function reviewSeries(runDir, stage, windowReviews, contractText, cycle) {
  ...
  const { session, metrics } = await createPiExperimentSession({
    runDir,
    role,
    systemPrompt: `${reviewPrompt(stage, contractText, loadManifest(runDir))}\n\n你是当前交付范围的终审，与创作 Agent 和窗口 Reviewer 使用全新的独立 session。窗口 findings 只是线索，必须复核后自行决定保留、删除或补充。每个 P2 必须选择 repair 或 accepted_non_blocking；P0/P1 的 disposition 固定填 repair。`,
    customTools: [submit],
...
export async function reviewStage(runDir, stage, { operatorNote = "" } = {}) {
  ...
  const priorFindingIds = fs.existsSync(priorFile)
    ? priorRepairFindingIds(readJson(priorFile), { repairPending: Boolean(operatorNote) && manifest.state === `${stage}_repairing` })
    : [];
