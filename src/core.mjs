...
export function checkStoryboard(markdown, canonicalNames = []) { const lines=markdown.split("\n").filter(Boolean); const i=lines.findIndex((l)=>l.startsWith("|") && cells(l).join("|")===STORYBOARD_HEADER.join("|")); if(i<0) return ["missing exact 7-column header"]; const rows=lines.slice(i+2).filter((l)=>l.startsWith("|")&&!/^\|\s*[-:]+/.test(l)); const errors=[...fixedEntityErrors(markdown,canonicalNames)]; let expected=1, total=0; if(rows.length<12||rows.length>24)errors.push(`shot count ${rows.length}`); for(const row of rows){const c=cells(row);if(c.length!==7){errors.push("row has wrong column count");continue;} const seq=Number(c[0].match(/(\d+)$/)?.[1]);if(seq!==expected)errors.push(`shot ${c[0]} expected ${expected}`);expected++;errors.push(...dialogueCellErrors(c[2],`镜头 ${c[0]}`));const d=Number(c[6].match(/\d+/)?.[0]);if(!(d>=3&&d<=10))errors.push(`invalid duration ${c[0]}`);total+=d;}if(!(total>=60&&total<=120))errors.push(`total duration ${total}`);return [...new Set(errors)]; }
...
export function markdownDelivery(runDir) { const m=loadManifest(runDir); const all=[]; const ledgerFile=path.join(runDir,"canonical","ledger.json"),charactersFile=path.join(runDir,"canonical","characters.md");const ledgerNames=fs.existsSync(ledgerFile)?readJson(ledgerFile).names||[]:[],names=canonicalPersonNames(fs.existsSync(charactersFile)?readText(charactersFile):"",ledgerNames);for(let ep=1;ep<=m.episodes;ep++){const f=path.join(runDir,"storyboard",`ep-${String(ep).padStart(2,"0")}.md`);if(!fs.existsSync(f))throw new Error(`missing storyboard ${ep}`);const md=readText(f);const errors=checkStoryboard(md,names);if(errors.length)throw new Error(`storyboard ${ep}: ${errors.join("; ")}`);all.push(md);}return all.join("\n\n---\n\n"); }
