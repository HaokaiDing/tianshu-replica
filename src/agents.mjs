...
function shortcutChecks(markdown) {
  const failures=[];
...
export async function produceStoryboards(runDir) {
  ...
  for(const b of batches(m.episodes)){let active={episode:b.from};const workDir=path.join(runDir,'work',`storyboard-${b.from}-${b.to}`);const runChecks=defineTool({name:'run_checks',label:'Check storyboard',description:'Validate the saved draft for the current episode before submission, including the target-market contract.',parameters:Type.Object({}),async execute(){const draft=path.join(workDir,'draft.md'),errors=fs.existsSync(draft)?[...checkStoryboard(readText(draft)),...marketChecks(readText(draft),market),...shortcutChecks(readText(draft))]:['尚未写入草稿'];return {content:[{type:'text',text:errors.length?`FAIL: ${errors.join('；')}`:'PASS: storyboard is eligible for submission'}],details:{errors},terminate:false};}});const submit=defineTool({name:"submit_storyboard",label:"Submit storyboard",description:"Submit the checked saved draft. The tool reads the saved draft; never paste a storyboard into this call.",parameters:Type.Object({episode:Type.Integer()}),async execute(_id,p){const draft=path.join(workDir,'draft.md');const errors=p.episode===active.episode&&fs.existsSync(draft)?[...checkStoryboard(readText(draft)),...marketChecks(readText(draft),market),...shortcutChecks(readText(draft))]:['wrong episode or missing draft'];if(errors.length)return {content:[{type:"text",text:`REJECTED ${errors.join("; ")}`}],details:{errors},terminate:false};const markdown=readText(draft);writeText(path.join(runDir,"storyboard",`ep-${ep(p.episode)}.md`),markdown);writeJson(taskPath(runDir,`storyboard-ep-${ep(p.episode)}`),{state:"passed",digest:sha(markdown)});return {content:[{type:"text",text:"ACCEPTED storyboard"}],details:{},terminate:true};}});
