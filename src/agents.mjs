...
export async function produceStoryboards(runDir) {
  ...
  for(const b of batches(m.episodes)){let active={episode:b.from};const workDir=path.join(runDir,'work',`storyboard-${b.from}-${b.to}`);const runChecks=defineTool({name:'run_checks',label:'Check storyboard',description:'Validate the saved draft for the current episode before submission.',parameters:Type.Object({}),async execute(){const draft=path.join(workDir,'draft.md'),errors=fs.existsSync(draft)?checkStoryboard(readText(draft)):['尚未写入草稿'];return {content:[{type:'text',text:errors.length?`FAIL: ${errors.join('；')}`:'PASS: storyboard is eligible for submission'}],details:{errors},terminate:false};}});const submit=defineTool({name:"submit_storyboard",label:"Submit storyboard",description:"Submit the checked saved draft. The tool reads the draft; never paste a storyboard into this call.",parameters:Type.Object({episode:Type.Integer()}),async execute(_id,p){const draft=path.join(workDir,'draft.md');const errors=p.episode===active.episode&&fs.existsSync(draft)?checkStoryboard(readText(draft)):['wrong episode or missing draft'];if(errors.length)return {content:[{type:"text",text:`REJECTED ${errors.join("; ")}`}],details:{errors},terminate:false};const markdown=readText(draft);writeText(path.join(runDir,"storyboard",`ep-${ep(p.episode)}.md`),markdown);writeJson(taskPath(runDir,`storyboard-ep-${ep(p.episode)}`),{state:"passed",digest:sha(markdown)});return {content:[{type:"text",text:"ACCEPTED storyboard"}],details:{},terminate:true};}});
    const tools=artifactTools(runDir,workDir), role=`storyboard-${b.from}-${b.to}`;
    const {session,metrics}=await createPiExperimentSession({runDir,role,systemPrompt:`你是天书分镜导演。每次任务必须严格走三步：1) 用 write_draft 写完整分镜；2) 调用 run_checks；3) 只有 PASS 后调用 submit_storyboard 并传当前集数。submit 会读取草稿，绝不把分镜粘进 submit 参数。不要输出解释。表头必须是：| ${STORYBOARD_HEADER.join(" | ")} |。每集 12-20 镜、单镜 3-7 秒、总时长 55-95 秒。资料不够时才用 read_artifact。`,customTools:[...tools,runChecks,submit],toolNames:["read_artifact","write_draft","run_checks","submit_storyboard"]});
...
export async function reviewScripts(runDir) {
  ...
  for(const b of batches(m.episodes)){
    ...
    const {session,metrics}=await createPiExperimentSession({runDir,role,systemPrompt:"You are a strict short-drama reviewer. Only report real P0/P1/P2 defects with episode numbers. P0 means upstream/systemic, P1 needs repair, P2 is a note. Submit through the tool.",customTools:[submit],toolNames:['submit_review']});
