...
function artifactTools(runDir, workDir) {
  ...
  const write=defineTool({name:'write_draft',label:'Write draft',description:'Write a draft only inside this task workspace.',parameters:Type.Object({markdown:Type.String({minLength:1})}),async execute(_id,p){writeText(path.join(workDir,'draft.md'),p.markdown);return {content:[{type:'text',text:'draft saved'}],details:{}};}});
  return [read,write];
}
...
export async function produceScripts(runDir) {
  ...
  for(const b of batches(m.episodes)){let active={episode:b.from};const submit=defineTool({name:"submit_screenplay",label:"Submit screenplay",description:"Submit one screenplay for the currently requested episode.",parameters:Type.Object({episode:Type.Integer(),markdown:Type.String({minLength:700})}),async execute(_id,p){if(p.episode!==active.episode)return {content:[{type:"text",text:"REJECTED wrong episode"}],details:{},terminate:false};if(!/【本集钩子】/.test(p.markdown)||!/【连续性检查】/.test(p.markdown))return {content:[{type:"text",text:"REJECTED missing hook or continuity"}],details:{},terminate:false};writeText(path.join(runDir,"screenplay",`ep-${ep(p.episode)}.md`),p.markdown);writeJson(path.join(runDir,"continuity",`ep-${ep(p.episode)}.json`),{episode:p.episode,digest:sha(p.markdown),hook:p.markdown.match(/【本集钩子】[^\n]*/)?.[0]||""});writeJson(taskPath(runDir,`screenplay-ep-${ep(p.episode)}`),{state:"passed",digest:sha(p.markdown)});return {content:[{type:"text",text:"ACCEPTED"}],details:{},terminate:true};}});
    const workDir=path.join(runDir,'work',`writer-${b.from}-${b.to}`), tools=artifactTools(runDir,workDir);
    const {session}=await createPiExperimentSession({runDir,role:`writer-${b.from}-${b.to}`,systemPrompt:"You are a Tianshu Writer. Produce one requested bilingual screenplay at a time. Read the injected plan and continuity; preserve names and facts. If you need more evidence, use read_artifact. You may use write_draft for scratch work. Submit through the tool only.",customTools:[...tools,submit],toolNames:["read_artifact","write_draft","submit_screenplay"]});
    try{for(let n=b.from;n<=b.to;n++){const task=taskPath(runDir,`screenplay-ep-${ep(n)}`);const artifact=path.join(runDir,"screenplay",`ep-${ep(n)}.md`);const prior=fs.existsSync(task)?JSON.parse(readText(task)):null;if(prior?.state==='passed'&&fs.existsSync(artifact))continue;active.episode=n;const prev=n>1&&fs.existsSync(path.join(runDir,"continuity",`ep-${ep(n-1)}.json`))?readText(path.join(runDir,"continuity",`ep-${ep(n-1)}.json`)):"start";const repair=prior?.repairInstruction?`\nRepair instruction:\n${prior.repairInstruction}`:"";await promptWithWatchdog(session,{prompts:0,turns:0,toolCalls:0,compactions:0,usage:[],events:[]},`Episode ${n}.\nPlan:\n${outline}\nCharacters:${chars}\nLedger:${ledger}\nPrevious continuity:${prev}${repair}`);}}finally{session.dispose();}}
...
export async function produceStoryboards(runDir) {
  ...
  for(const b of batches(m.episodes)){let active={episode:b.from};const submit=defineTool({name:"submit_storyboard",label:"Submit storyboard",description:"Submit a fixed 11-column storyboard for the current episode.",parameters:Type.Object({episode:Type.Integer(),markdown:Type.String({minLength:500})}),async execute(_id,p){const errors=p.episode===active.episode?checkStoryboard(p.markdown):["wrong episode"];if(errors.length)return {content:[{type:"text",text:`REJECTED ${errors.join("; ")}`}],details:{errors},terminate:false};writeText(path.join(runDir,"storyboard",`ep-${ep(p.episode)}.md`),p.markdown);writeJson(taskPath(runDir,`storyboard-ep-${ep(p.episode)}`),{state:"passed",digest:sha(p.markdown)});return {content:[{type:"text",text:"ACCEPTED"}],details:{},terminate:true};}});
    const workDir=path.join(runDir,'work',`storyboard-${b.from}-${b.to}`), tools=artifactTools(runDir,workDir);
    const {session}=await createPiExperimentSession({runDir,role:`storyboard-${b.from}-${b.to}`,systemPrompt:`You are a Tianshu Storyboard Director. Produce exactly this header: | ${STORYBOARD_HEADER.join(" | ")} |. 12-20 shots, each 3-7 sec, total 55-95 sec. Use read_artifact if the supplied screenplay is insufficient; write_draft is only for scratch. Submit only through tool.`,customTools:[...tools,submit],toolNames:["read_artifact","write_draft","submit_storyboard"]});
    try{for(let n=b.from;n<=b.to;n++){const task=taskPath(runDir,`storyboard-ep-${ep(n)}`);const artifact=path.join(runDir,"storyboard",`ep-${ep(n)}.md`);const prior=fs.existsSync(task)?JSON.parse(readText(task)):null;if(prior?.state==='passed'&&fs.existsSync(artifact))continue;active.episode=n;await promptWithWatchdog(session,{prompts:0,turns:0,toolCalls:0,compactions:0,usage:[],events:[]},`Episode ${n} screenplay:\n${readText(path.join(runDir,"screenplay",`ep-${ep(n)}.md`))}`);}}finally{session.dispose();}}
...
export function applyRepair(runDir) {
  ...
  for(const finding of plan.findings||[]){const id=`screenplay-ep-${ep(finding.episode)}`;const file=taskPath(runDir,id);const prior=fs.existsSync(file)?JSON.parse(readText(file)):{id};writeJson(file,{...prior,state:'stale',repairInstruction:finding.reason});if(!stale.includes(id))stale.push(id);}
